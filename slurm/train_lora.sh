#!/bin/bash -l
# Seed array for GRPO of plain LoRA. Usage: MODEL=qwen2.5-7b TASK=hard CFG=r32 sbatch [-a 0] slurm/train_lora.sh [extra train_lora args]
# CFG=r<rank> names the run dir (lora-grpo/<cfg>) and sets --rank. 1.5B fits an L4/A40: sbatch --constraint='l4|a40|l40s|a100-40g|a100-80g|h100'
#SBATCH -J train_lora
#SBATCH -a 0-2
...
OUT="outputs/runs/$("$HOME/.local/bin/uv" run python -c "from turbolora.models import MODELS; print(MODELS['${MODEL:?}'].family)")/${MODEL}/lora-grpo/${CFG}/seed${SLURM_ARRAY_TASK_ID:?}"
...
"$HOME/.local/bin/uv" run -m turbolora.train_lora --model "$MODEL" --task "${TASK:?}" --out "$OUT" --seed "$SLURM_ARRAY_TASK_ID" --rank "${BASH_REMATCH[1]}" "$@" &
pid=$! status=0
