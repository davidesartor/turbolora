#!/bin/bash -l
# Seed array for GRPO of TinyLoRA. Usage: MODEL=qwen2.5-7b TASK=hard CFG=r2-u8 sbatch [-a 0] slurm/train_tinylora.sh [--untie ...]
# CFG=r<rank>-u<proj_dim> names the run dir (tinylora-grpo/<cfg>) and sets --rank/--proj-dim. 1.5B fits an L4/A40: sbatch --constraint='l4|a40|l40s|a100-40g|a100-80g|h100'
#SBATCH -J train_tinylora
#SBATCH -a 0-2
...
[[ "${CFG:?}" =~ ^r([0-9]+)-u([0-9]+) ]] || { echo "CFG must be r<rank>-u<proj_dim>, got $CFG" >&2; exit 1; }

# run dir tagged into the job comment (sweep.py reads it via squeue %k)
OUT="outputs/runs/$("$HOME/.local/bin/uv" run python -c "from turbolora.models import MODELS; print(MODELS['${MODEL:?}'].family)")/${MODEL}/tinylora-grpo/${CFG}/seed${SLURM_ARRAY_TASK_ID:?}"
...
"$HOME/.local/bin/uv" run -m turbolora.train_tinylora --model "$MODEL" --task "${TASK:?}" --out "$OUT" --seed "$SLURM_ARRAY_TASK_ID" --rank "${BASH_REMATCH[1]}" --proj-dim "${BASH_REMATCH[2]}" "$@" &
pid=$! status=0
