...
Objective = Callable[[Float[Tensor, "T"], int], tuple[float, float]]  # (θ, trial) -> (mean, SEM)
...
class HeteroskedasticGP(SingleTaskGP):
    ...
    def __init__(
        ...
        d = train_X.shape[-1]
        X_scaler, Y_scaler, log_Yvar_scaler = Normalize(d=d), Standardize(m=1), Standardize(m=1)
        X, (Y, _), (log_Yvar, _) = X_scaler(train_X), Y_scaler(train_Y), log_Yvar_scaler(train_Yvar.log())
        for scaler in (X_scaler, Y_scaler, log_Yvar_scaler):
            scaler.eval()

        # noise GP: variance of a log sample variance over n samples ≈ 2/(n-1)
        noise_likelihood = GaussianLikelihood()
        noise_likelihood.noise = torch.tensor(2.0 / (n_samples - 1), dtype=torch.float64)
        noise_model = SingleTaskGP(X, log_Yvar, likelihood=noise_likelihood)
        fit_gpytorch_mll(ExactMarginalLogLikelihood(noise_model.likelihood, noise_model))
        noise_model.eval().requires_grad_(False)
        with torch.no_grad():
            predicted_var = log_Yvar_scaler.untransform(noise_model.posterior(X).mean)[0].exp()

        # objective GP with that noise fixed (in standardized-Y units)
        super().__init__(
            X,
            Y,
            likelihood=FixedNoiseGaussianLikelihood(noise=predicted_var.squeeze(-1) / Y_scaler._stdvs_sq[0]),
    ...
    def posterior(self, X: Float[Tensor, "M T"], **_) -> GPyTorchPosterior:
        self.eval()
        return self.Y_scaler.untransform_posterior(GPyTorchPosterior(self(self.X_scaler(X))))
...
def argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--theta-max", type=float, default=0.1)
    parser.add_argument("--n-baseline", type=int, default=10, help="θ=0 replicates that seed the noise model")
    parser.add_argument("--n-sobol", type=int, default=10, help="quasi-random trials before the GP takes over")
    parser.add_argument("--n-evals", type=int, default=600, help="non-baseline trials")
    parser.add_argument("--thompson-candidates", type=int, default=2000)
    return parser


def search(args: argparse.Namespace, objective: Objective, dim: int, n_samples: int, out: Path) -> dict | None:
    """Maximize `objective` over [θ_min, θ_max]^dim, logging to `<out>/trials.json`; returns the evaluated non-baseline trial with the best GP posterior mean.
    ...
    bounds = torch.tensor([[args.theta_min] * dim, [args.theta_max] * dim], dtype=torch.float64)
    ...
    while len(trials) < args.n_baseline + args.n_evals and not stop.requested:
        ...
        if i < args.n_baseline:
            ...
            candidates = bounds[0] + (bounds[1] - bounds[0]) * torch.rand(args.thompson_candidates, dim, dtype=torch.float64)
            with torch.no_grad():
                theta = candidates[gp.posterior(candidates).sample().argmax()]
        value, sem = objective(theta, i)
        trials.append(dict(trial=i, baseline=i < args.n_baseline, theta=theta.tolist(), value=value, sem=sem))
        trials_path.write_text(json.dumps(trials, indent=1))
        best = max(t["value"] for t in trials)
        print(f"trial {i}: {value:.4f} ± {sem:.4f} (best {best:.4f}) θ={[round(x, 4) for x in theta.tolist()]}")
    if stop.requested:
        print("stopped on signal; rerun to resume")
        return None

    # final pick: denoised best-observed
    searched = [t for t in trials if not t["baseline"]]
    gp = fit_gp(trials, n_samples)
    with torch.no_grad():
        means = gp.posterior(torch.tensor([t["theta"] for t in searched], dtype=torch.float64)).mean.squeeze(-1)
    chosen = searched[int(means.argmax())]
    print(f"selected trial {chosen['trial']}: observed {chosen['value']:.4f}, posterior mean {means.max():.4f}, θ={chosen['theta']}")
    return dict(chosen, steps=len(trials))
